...
/*
*  @brief Path type is a string wrapper to use only for path directories 
*/
...
namespace std {
template<>
struct hash<shEngineSDK::Path>
{
  shEngineSDK::SIZE_T operator()(const shEngineSDK::Path& path) const
  {
    shEngineSDK::SIZE_T hash = 0;
    for (char i : path) {
      hash = 65599 * hash + i;
    }
